...
class VideoPage extends Component {
    ...
    handleCensor_toggle() {
        Axios.get(`${config.api}/cen.php?id=${this.state.video.id}`)
            .then(() => {
                this.setState(prevState => {
    ...
    handleBookmark_add(category, star = null) {
        ...
        if (time) {
            if (star === null) {
                ...
                Axios.get(`${config.api}/addbookmark.php?videoID=${this.state.video.id}&categoryID=${category.id}&time=${time}&starID=${star.id}`)
                    .then(({data}) => success(data))
            }
        }
            ...
            <div className="video-page col-12 row">
                <section className="col-9">
                        ...
                        <div className="col-12">
                            <h1 className="header__title h2 align-middle">
                                ...
                                <ContextMenu id="title">
                                    ...
                                    ><i className="far fa-copy"/> Copy Franchise</MenuItem>
                                </ContextMenu>
                    ...
                    <div className="video-container" onWheel={this.handleWheel.bind(this)}>
                        ...
                        <ContextMenu id="video">
                            ...
                            >{this.state.video.censored
                                ? <React.Fragment><i className="far fa-check-circle"/> Uncensor</React.Fragment>
                                : <React.Fragment><i className="far fa-exclamation-circle"/> Censor</React.Fragment>
                            }</MenuItem>

                            <MenuItem onClick={() => this.handlePlays_reset()}
                            ><i className="far fa-trash-alt"/> Remove Plays</MenuItem>

                            <hr/>

                            <MenuItem onClick={() => this.handleFname_copy()}
                            ><i className="far fa-copy"/> Copy Filename</MenuItem>
                        </ContextMenu>
                    </div>
                        ...
                        {this.state.loaded.bookmarks && this.state.loaded.video && Object.keys(this.state.bookmarks).map((i) => (
                            <React.Fragment key={i}>
                                <div className="btn btn-sm btn-outline-primary bookmark"
                                     style={{left: `${(this.state.bookmarks[i].start * 100 / this.state.video.duration) * 0.88}%`}}
                                     onClick={() => this.handleVideo_play(this.state.bookmarks[i].start)}
                                     ref={(bookmark => this.bookmarks[i] = bookmark)} data-level={1}
                                >
                                    ...
                                    <ReactTooltip id={`bookmark-info-${i}`} effect="solid">
                                        <img alt="star" className="star__image"
                                        ...
                                        {Object.keys(this.state.bookmarks[i].attributes).map((b_i) => (
                                            <div key={b_i} className="attribute btn btn-sm btn-light">
                                ...
                                <ContextMenu id={`bookmark-${i}`}>
                                    ...
                                    <MenuItem
                                        onClick={() => this.handleBookmark_time(this.state.bookmarks[i].id)}
                                    ><i className="far fa-clock"/> Change Time</MenuItem>

                                    <MenuItem
                                        onClick={() => this.handleBookmark_remove(this.state.bookmarks[i].id)}
                                    ><i className="far fa-trash-alt"/> Delete</MenuItem>
                                </ContextMenu>
                            </React.Fragment>
                        ))}
                    </div>
                ...
                <aside className="col-3">
                    <div id="stars" className="row justify-content-center">
                        {this.state.loaded.stars && Object.keys(this.state.stars).map((key, i) => (
                            <React.Fragment key={i}>
                                <div className="star col-4">
                                    <ContextMenuTrigger id={`star-${i}`}>
                                        ...
                                        <a href={`${config.source}/star.php?id=${this.state.stars[i].id}`}
                                           className="star__name d-block">{this.state.stars[i].name}</a>
                                    </ContextMenuTrigger>
                                </div>
                                ...
                                <ContextMenu id={`star-${i}`}>
                                    <MenuItem
                                        onClick={() => {
                                            this.handleModal("Add Bookmark",
                                                Object.keys(this.state.categories).map((c_i) => {
                                                    return (
                                                        <div key={c_i}
                                                             className="btn btn-sm btn-outline-primary d-block w-auto"
                                                             onClick={() => {
                                                                 this.handleModal()
                                                                 this.handleBookmark_add(this.state.categories[c_i], this.state.stars[i])
                                                             }}
                                                        >{this.state.categories[c_i].name}</div>
                                                    )
                                                })
                                    ...
                                    <MenuItem
                                    ><i className="far fa-plus"/> Add Global Attribute</MenuItem>

                                    <MenuItem
                                        onClick={() => this.handleStar_remove(this.state.stars[i].id)}
                                    ><i className="far fa-trash-alt"/> Remove</MenuItem>
                                </ContextMenu>
                            </React.Fragment>
                        ))}
                    </div>
