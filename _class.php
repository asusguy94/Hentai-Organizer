...
class Basic
{
    public function head($title = '', $stylesheet = '', $script = '')
    {
        $this->title($title);
    ...
    public function stylesheet($data)
    {
    ...
    public function pathToFname($path)
    {
    ...
    public function generateName($fname)
    {
        return explode('.' . $this->getExtension($fname), $fname)[0];
    }

    public function generateFranchise($fname)
    {
        return explode(' Episode', $fname)[0];
    }

    public function generateEpisode($fname)
    {
    ...
    public static function getClosestQ($quality)
    {
        if ($quality == 396) {
            return 480;
        }
    ...
    public static function removeExtensionPath($path)
    {
        return substr($path, 0, strrpos($path, "."));
    }

    public static function file_check($filename, $videoID, $isRoot = true)
    {
...
class HomePage
{
    ...
    public function popular()
    {
        ...
        $query = $pdo->prepare("
											...
											WHERE noStar = FALSE
											GROUP BY videoID
											ORDER BY total DESC, plays.time DESC
										");
        $query->execute();
    ...
    public function printData($input, $ribbonCol = null)
    {
        $count = $this->count;
        print "<div class='row'>";
            ...
            if ($count > 0) {
                ...
                print sprintf("
						<a class='video $col px-0 mx-3 ribbon-container' href='video.php?id=$data[id]'>
							<img class='mx-auto img-thumbnail' src='images/videos/$data[id]-%d.jpg' alt='thumbnail'>
							<span class='title mx-auto d-block'>$data[name]</span>", THUMBNAIL_RES);
                if ($ribbonCol) {
                    print "<span class='ribbon'>$data[$ribbonCol]</span >";
                }
...
class DB
{
    public function videoExists($path)
    {
        global $pdo;
...
class Star
{
    ...
    public function starExists($name)
    {
        global $pdo;
    ...
    public function getRelations($id)
    {
        global $pdo;
        $query = $pdo->prepare("
											SELECT starrelations.id, otherstarID, starID, stars.name, starrelations.name AS type
											FROM starrelations
    ...
    public function relationExists($starID, $otherID)
    {
        global $pdo;
    ...
    public function getRelations_other($id)
    {
        global $pdo;
        $query = $pdo->prepare("
											SELECT starrelations.id, otherstarID, starID, stars.name, starrelations.name AS type
        ...
        foreach ($query->fetchAll() as $relation) {
            ...
            if ($this->relationExists($id, $otherStarID)) {
                continue;
            }
            ...
            echo "
					...
					<div class='col-12'>
                    	<button class='btn btn-primary relation-add'><span class='fas fa-check'></span>Add</button>
                    </div>
				";
        }
    }

    public function getBookmarks($id, $videoID)
    {
        global $pdo;
    ...
    public function saveProperty($starID)
    {
        global $pdo;
    ...
    public function fetchAttributes($starID)
    {
        global $pdo;
    ...
    public function addAttribute($starID)
    {
        global $pdo;
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    public function fetchVideos($starID)
    {
        global $pdo;
    ...
    public function bookmarkStarCount($id, $videoID)
    {
        global $pdo;
...
class Video
{
    ...
    public function sql($override = 0)
    {
    ...
    public function fetchVideos($limit, $config = [])
    {
        ...
        if ($query->rowCount()) {
            ...
            foreach ($query->fetchAll() as $data) {
                ...
                if (!Basic::file_check($data['path'], $data['id'])) {
                    continue;
                }
    ...
    public function nextVideo($id)
    {
        ...
        if ($this->sqlMethod === 'all') {
            $query = $pdo->prepare("{$this->sql()} ORDER BY franchise, episode");
        } else {
            $query = $pdo->prepare("{$this->sql()} OR videos.id = :videoID ORDER BY franchise, episode");
        }
    ...
    public function idToFranchise($id)
    {
        global $pdo;
    ...
    public function getPlays($videoID)
    {
        global $pdo;
    ...
    public function fetchFranchise($videoID)
    {
        global $pdo;
    ...
    public function fetchInfo_sidebar($id)
    {
    ...
    public function getOffset($start)
    {
    ...
    public function fetchBookmarks($id)
    {
        global $pdo;
    ...
    public function addCategory_form($id)
    {
        global $pdo;
    ...
    public function addStar_form($id)
    {
        global $pdo;
    ...
    public function fetchAttributes($id)
    {
        ...
        global $pdo;
        if (!$len) {
            $query = $pdo->prepare($sql_noStar);
        } else {
            $query = $pdo->prepare($sql_star);
        }
    ...
    public function hiddenData()
    {
        ...
        if ($query->rowCount()) {
            ...
            if (file_exists("vtt/$_GET[id].vtt")) {
                echo '<div id="vtt"></div>';
            }
...
class Franchise
{
    ...
    public function sql()
    {
        if ($this->sqlMethod == 'somthing') {
            $sql = "SELECT SOMTHING";
        } else if ($this->sqlMethod == 'somthing eles') {
            $sql = "SELECT SOMTHING ELSE";
        } else {
            $sql = "SELECT franchise FROM videos GROUP BY franchise ORDER BY franchise";
        }

        return $sql;
    }

    public function fetchFranchises()
    {
        global $pdo;
        $query = $pdo->prepare($this->sql());
        $query->execute();
...
class FFMPEG
{
    ...
    public function getDuration($fname)
    {
    ...
    public function generateThumbnail($fname, $videoID, $startTime = 0, $width = 0)
    {
        $input = "videos/$fname";
    ...
    public function getVideoHeight($fname)
    {
        $input = "videos/$fname";
        if ($this->rootFix) {
            $input = "../$input";
...
class VTT extends FFMPEG
{
    ...
    public function generateVtt($fname, $videoID, $width = 360)
    {
        /* Variables */
        $input = "videos/$fname";
...
class Settings
{
    ...
    public static function setSettings($nameArr, $valueArr)
    {
        global $pdo;
