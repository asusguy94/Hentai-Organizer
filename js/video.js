...
function addPlay() {
    ajax_post('ajax/video_addplay.php', [{ videoID: videoID }], function () {
...
function renameVideo(videoName) {
    ajax(
        'ajax/rename_video.php',
        `videoID=${videoID}&videoName=${encodeURIComponent(videoName)}`
    )
}

function renameFranchise(franchiseName) {
    ajax(
        'ajax/rename_franchise.php',
        `videoID=${videoID}&franchiseName=${encodeURIComponent(franchiseName)}`
    )
}
...
function addBookmark(categoryID, videoID) {
    ...
    localStorage.bookmark = seconds
    ajax(
        'ajax/add_bookmark.php',
        `seconds=${seconds}&categoryID=${categoryID}&videoID=${videoID}`
    )
}

function bookmark_editCategory(bookmarkID, categoryID) {
    ajax(
        'ajax/bookmark_editCategory.php',
        `bookmarkID=${bookmarkID}&categoryID=${categoryID}`
    )
}
...
function bookmark_editTime(bookmarkID) {
    ...
    localStorage.bookmark = seconds
    ajax(
        'ajax/bookmark_editTime.php',
        `bookmarkID=${bookmarkID}&seconds=${seconds}`
    )
}
...
function removeVideoCategory(videoID, categoryID) {
    ajax(
        'ajax/remove_videocategory.php',
        `videoID=${videoID}&categoryID=${categoryID}`
    )
}
...
function addBookmarkStar(bookmarkID, starID) {
    ajax(
        'ajax/add_bookmark_star.php',
        `bookmarkID=${bookmarkID}&starID=${starID}`
    )
}

function removeBookmarkStar(bookmarkID, starID) {
    ajax(
        'ajax/bookmark_removeStar.php',
        `bookmarkID=${bookmarkID}&starID=${starID}`
    )
}
...
function addBookmark_and_star(categoryID, starID) {
    ...
    localStorage.bookmark = seconds
    ajax(
        'ajax/add_bookmark_and_star.php',
        `categoryID=${categoryID}&starID=${starID}&seconds=${seconds}&videoID=${videoID}`
    )
}

function addGlobal_starAttribute(starID, attributeID) {
    ajax(
        'ajax/video_add_starattribute.php',
        `videoID=${videoID}&starID=${starID}&attributeID=${attributeID}`
    )
}
...
function addBookmarkAttribute(bookmarkID, attributeID) {
    ajax(
        'ajax/add_bookmark_attribute.php',
        `bookmarkID=${bookmarkID}&attributeID=${attributeID}`
    )
}

function generateThumbnail(seconds) {
    ajax(
        'ajax/video_generatethumbnail.php',
        `videoID=${videoID}&seconds=${Math.round(seconds)}`
    )
}
...
function editDate($date) {
    ajax_post('ajax/video_setdate.php', [{ videoID: videoID }, { date: $date }])
}
    ...
    $.contextMenu({
        ...
        items: {
            rename_title: {
                ...
                callback: function () {
                    $('body').append(
                        '<div id="dialog" title="Edit Video"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        $('#dialog').append(
                            '<input type="text" name="videoName_edit" value="' +
                                $('#video > h2 #video-name').text() +
                                '" autofocus>'
                        )
                        ...
                        document
                            .querySelector('input[name="videoName_edit"]')
            ...
            rename_framchise: {
                ...
                callback: function () {
                    $('body').append(
                        '<div id="dialog" title="Edit Franchise"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        $('#dialog').append(
                            `<input type="text" value="${$(
                                ...
                                'data-franchise'
                            )}" name="videoFranchise_edit" autofocus>`
                        )
                        ...
                        input[0].setSelectionRange(len, len)
                        document
                            .querySelector('input[name="videoFranchise_edit"]')
                            .addEventListener('keydown', function (e) {
                                if (e.keyCode === 13)
            ...
            add_alias: {
                ...
                callback: function () {
                    $('body').append(
                        '<div id="dialog" title="Add Alias"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        $('#dialog').append(
                            '<input type="text" name="videoAlias_add" autofocus>'
                        )
                        $('input[name="videoAlias_add"]')[0].focus()
                        document
                            .querySelector('input[name="videoAlias_add"]')
                            .addEventListener('keydown', function (e) {
        ...
        items: {
            rename_alias: {
                ...
                callback: function (itemKey, options) {
                    ...
                    $('body').append(
                        '<div id="dialog" title="Edit Alias"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        $('#dialog').append(
                            `<input type="text" name="videoAlias_edit" value="${aliasText_current}" autofocus>`
                        )
                        ...
                        input[0].setSelectionRange(len, len)
                        document
                            .querySelector('input[name="videoAlias_edit"]')
                            .addEventListener('keydown', function (e) {
                                if (e.keyCode === 13) {
        ...
        items: {
            ...
            rename_file: {
                ...
                callback: function () {
                    ...
                    $('body').append(
                        '<div id="dialog" title="Edit File"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        $('#dialog').append(
                            '<input type="text" name="videoFile_edit" value="' +
                                videoPath_current +
                                '" autofocus>'
                        )
                        ...
                        input[0].setSelectionRange(len, len)
                        document
                            .querySelector('input[name="videoFile_edit"]')
                            .addEventListener('keydown', function (e) {
            ...
            copy_filename: {
                ...
                icon: 'copy',
                callback: function () {
                    let videoPath_current = $(videoPlayer)
                        .find('source')
                        .first()
                        .attr('src')
                        .split('/')[1]
        ...
        items: {
            ...
            remove_star: {
                ...
                callback: function (itemKey, options) {
                    let id = options.$trigger.attr('data-bookmark-id')
                    let starID = $(
                        `.bookmark-info[data-bookmark-id="${id}"] > .star-image`
                    ).attr('data-star-id')
            ...
            add_attribute: {
                ...
                callback: function (itemKey, options) {
                    ...
                    $('body').append(
                        '<div id="dialog" title="Add Attribute"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                            ...
                            width: 250,
                            maxHeight: $(window).height() - 55,
                            position: { my: 'top', at: 'top+50' },
                        })
                        let selectedAttribute_query = $(
                            `.bookmark-info[data-bookmark-id="${bookmarkID}"] .btn[data-attribute-id]`
                        )
                        let selectedAttribute = []
                        for (
                            let i = 0;
                            i < selectedAttribute_query.length;
                            i++
                        ) {
                            let name = $(selectedAttribute_query)
                                .eq(i)
                        ...
                        for (let i = 0; i < attribute_query.length; i++) {
                            let attributeID = $(attribute_query)
                                .eq(i)
                                .attr('data-attribute-id')
                            let attributeName = $(attribute_query).eq(i).text()

                            if (selectedAttribute.indexOf(attributeID) === -1)
                                $('#dialog').append(
                                    `<div class="btn unselectable" onclick="addBookmarkAttribute(${bookmarkID}, ${attributeID})">${attributeName}</div>`
                                )
                        }
                        searchData()
                    })
            ...
            edit: {
                ...
                callback: function (itemKey, options) {
                    ...
                    $('body').append(
                        '<div id="dialog" title="Change Category"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        for (
                            ...
                            if (categoryName !== bookmarkName)
                                $('#dialog').append(
                                    `<div class="btn unselectable" onclick="bookmark_editCategory(${bookmarkID},${categoryID})">${categoryName}</div>`
                                )
                        }
                        searchData()
                    })
        ...
        items: {
            add_bookmark: {
                ...
                callback: function (itemKey, options) {
                    ...
                    $('body').append(
                        '<div id="dialog" title="Add Bookmark"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        for (
                            ...
                            let categoryID = $('#category_list > option')
                                .eq(i)
                                .attr('data-category-id')
                            let categoryName = $('#category_list > option')
                                .eq(i)
                                .attr('value')
                            $('#dialog').append(
                                `<div class="btn unselectable" onclick="addBookmark_and_star(${categoryID}, ${starID})">${categoryName}</div>`
                            )
                        }
                        searchData()
                    })
                ...
                callback: function (itemKey, options) {
                    ...
                    $(function () {
                        ...
                        for (let i = 0; i < attribute_query.length; i++) {
                            ...
                            let attributeName = $(attribute_query).eq(i).text()
                            $('#dialog').append(
                                `<div class="btn unselectable" onclick="addGlobal_starAttribute(${starID}, ${attributeID})">${attributeName}</div>`
                            )
                        }
                        searchData()
                    })
        ...
        items: {
            edit_date: {
                ...
                callback: function () {
                    ...
                    $('body').append(
                        '<div id="dialog" title="Edit Date"></div>'
                    )
                    $(function () {
                        $('#dialog').dialog({
                            close: function () {
                        ...
                        $('#dialog').append(
                            `<input type="text" name="date_edit" value="${date_current}" autofocus>`
                        )
                        ...
                        input[0].setSelectionRange(len, len)
                        document
                            .querySelector('input[name="date_edit"]')
                            .addEventListener('keydown', function (e) {
...
function collisionCheck(a, b) {
    ...
    return (
        !(a.x + a.width < b.x - distance_min.x) ||
        a.x - distance_min.x > b.x + b.width
    )
}
    ...
    for (let i = 0; i < starClass.length; i++) {
        starClass[i].addEventListener('mouseenter', function () {
            ...
            for (
                let j = 0;
                j <
                $(`.bookmark-info > .star-image[data-star-id="${starID}"]`)
                    .length;
                j++
            ) {
                let bookmarkID = $(
...
function attributeTooltip() {
    ...
    for (let i = 0; i < attributeClass.length; i++) {
        attributeClass[i].addEventListener('mouseenter', function () {
            let attributeID = attributeClass[i].getAttribute(
                'data-attribute-id'
            )
            for (
                let j = 0;
                j <
                $(`.bookmark-info > [data-attribute-id="${attributeID}"]`)
                    .length;
                j++
            ) {
                let bookmarkID = $(
...
function searchData() {
    ...
    document.addEventListener('keydown', (e) => {
        if (
            (e.which === CHAR_BACKSPACE && input.length) ||
            e.which === CHAR_SPACE ||
            (e.which >= CHAR_A && e.which <= CHAR_Z)
        ) {
            ...
            if (e.which === CHAR_BACKSPACE) {
                ...
                updateLabel(
                    (input += String.fromCharCode(e.which).toLowerCase())
                )
            }
